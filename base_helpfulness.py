import data_manip_helpfulness as data_manip
import random
...
def main():
    ...
    for i in range(1, 6):
        rating = [i]
        normalized_data = data_manip.generate_markov_chain\
            (pulled_data[0], word_list, pulled_data[1], rating, pulled_data[2], 0.5)
        print("--- " + str(i) + " Star Reviews ---")
        file_name = (str(i) + "starreviews.txt")
        review_generator(normalized_data, num_reviews, max_length, word_list,
                         num_sentence, file_name)
